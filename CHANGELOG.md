...
- expanded install, update, troubleshooting, and getting-started docs so runtime refresh behavior, release layout, sidecar recovery, and common in-session commands are easier to follow
...
- installer/runtime helper regressions are now covered by tests for source archive selection, centralized launcher/manifest path plus metadata semantics, launcher/manifest/service discovery precedence, child apply-result parsing and runner cleanup/spawn semantics, installed-runtime finalization orchestration, builtin changelog and resume behavior, model command edge cases, `install.sh` bootstrap behavior, app assembly entrypoints, runtime release publishing, launcher and installed-doc output paths, local and elevated installer file-helper semantics, consolidated installer service routing, cross-user installer privilege prediction, updater launcher-owner vs daemon-target messaging, sidecar lifecycle handling, package/bin distribution contracts, and public README/doc link consistency
