...
- installer/runtime helper regressions are now covered by tests for source archive selection, service path derivation, child apply-result parsing, builtin changelog and resume behavior, model command edge cases, `install.sh` bootstrap behavior, app assembly entrypoints, runtime release publishing, launcher and installed-doc output paths, sidecar lifecycle handling, package/bin distribution contracts, and public README/doc link consistency
