...
function renderRelatedContext(
  ...
  return [`- ${doc.title} (${reason || "related"})`, excerpt]
    .filter(Boolean)
    .join("\n  ");
}
...
export function compileFromDocsAndEvents(
  ...
  const historyLimit = Math.max(
    0,
    Number(params.historyLimit == null ? 3 : params.historyLimit) || 3,
  );
  ...
  const episodeDocs = !query
    ? []
    : docs
        .filter(
          (doc) => doc.exposure === "recall" && doc.tags.includes(EPISODE_TAG),
        )
  ...
  const recallDocs = !query
    ...
    : docs
        ...
        .filter(
          (doc) =>
            episodeDocs.length === 0 || !doc.tags.includes(CHRONICLE_TAG),
        )
        .filter(
          (doc) =>
  ...
  const relatedDocs = (() => {
    ...
    for (const edge of graph.edges) {
      if (
        ![...expandedProgressives, ...recallDocs, ...episodeDocs].some(
          (doc) => doc.id === edge.from,
        )
      )
        continue;
      ...
      if (
        recallDocs.some((item) => item.id === doc.id) ||
        episodeDocs.some((item) => item.id === doc.id) ||
        expandedProgressives.some((item) => item.id === doc.id)
      )
        continue;
    ...
    return out
      ...
      .map((row) => ({ reason: row.reason, doc: row.doc }));
  })();

  const history =
    !query || !shouldInjectRecentHistory(query)
      ? []
      : events
  ...
  return {
    ...
    progressive_expanded: expandedProgressives
      ...
      .join("\n\n"),
    episode_context: episodeDocs
      .map((doc) => renderExpandedDoc(doc))
      .join("\n\n"),
    recall_context: recallDocs
      .map((doc) => renderRecallContext(doc, query))
      .join("\n\n"),
    related_context: relatedDocs
      .map((row) => renderRelatedContext(row.doc, row.reason, query))
      .join("\n\n"),
    ...
    related_docs: relatedDocs.map((row) => ({
      reason: row.reason,
