...
import type { MemoryDoc, MemoryExposure, MemoryFidelity, MemoryKind, MemoryScope, MemoryStatus } from './types.js'
import { nowIso, normalizeList, safeString, slugify } from './utils.js'

export function ensureExposure(value: string, fallback: MemoryExposure = 'recall'): MemoryExposure {
  const normalized = safeString(value).trim()
  if (normalized === 'resident' || normalized === 'progressive' || normalized === 'recall') return normalized
  return fallback
}

export function ensureFidelity(value: string, fallback: MemoryFidelity = 'fuzzy'): MemoryFidelity {
  const normalized = safeString(value).trim()
  if (normalized === 'exact' || normalized === 'fuzzy') return normalized
  return fallback
}

export function ensureScope(value: string, fallback: MemoryScope = 'project'): MemoryScope {
  const normalized = safeString(value).trim()
  if (normalized === 'global' || normalized === 'domain' || normalized === 'project' || normalized === 'session') return normalized
  return fallback
}

export function ensureKind(value: string, fallback: MemoryKind = 'knowledge'): MemoryKind {
  const normalized = safeString(value).trim()
...
export function ensureStatus(value: string, fallback: MemoryStatus = 'active'): MemoryStatus {
  const normalized = safeString(value).trim()
  if (normalized === 'active' || normalized === 'superseded' || normalized === 'invalidated') return normalized
  return fallback
}

export function normalizeFrontmatter(raw: Record<string, any>, filePath: string, content: string): MemoryDoc {
