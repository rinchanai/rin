...
export type MemoryDocIndexSearchOptions = {
  limit?: number;
  exposure?: string;
};

export type MemoryDocIndexSearchResult = {
  doc: MemoryDoc;
  score: number;
  query: string;
};
...
export function memoryDocsDbPath(rootDir: string): string {
...
function searchWithDatabase(
  ...
  return [...candidates.entries()]
    ...
    .filter((row): row is MemoryDocIndexSearchResult => Boolean(row))
    .sort(
      (a, b) =>
        b.score - a.score ||
...
export function searchMemoryDocsWithTempIndex(
  docs: MemoryDoc[],
  query: string,
...
export function syncMemoryDocIndex(rootDir: string): void {
  const docs = loadMemoryDocsSync(rootDir).filter(
    (doc) => doc.status === "active" && doc.exposure === "memory_docs",
  );
...
export function searchIndexedMemoryDocs(
  ...
  syncMemoryDocIndex(rootDir);
  const db = createDatabase(memoryDocsDbPath(rootDir));
  try {
    return searchWithDatabase(
      db,
      query,
      Math.max(1, Number(options.limit || 8) || 8),
    ).filter(
      (row) =>
        !safeString(options.exposure || "").trim() ||
        row.doc.exposure === safeString(options.exposure || "").trim(),
    );
  } finally {
    db.close();
  }
