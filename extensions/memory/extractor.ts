...
function normalizeCandidate(raw: any): MemoryCandidate | null {
  const exposure = safeString(raw?.exposure).trim() as MemoryCandidate['exposure']
  if (!['resident', 'progressive', 'recall'].includes(exposure || '')) return null
  const candidate: MemoryCandidate = {
    ...
    tags: Array.isArray(raw?.tags) ? raw.tags.map((item: any) => safeString(item).trim()).filter(Boolean) : [],
    triggers: Array.isArray(raw?.triggers) ? raw.triggers.map((item: any) => safeString(item).trim()).filter(Boolean) : [],
  }
...
async function saveCandidate(candidate: MemoryCandidate, source: string) {
  ...
  return await executeMemoryTool({
    ...
    scope: candidate.scope || (candidate.exposure === 'progressive' ? 'domain' : 'project'),
    kind: candidate.kind || (candidate.exposure === 'progressive' ? 'preference' : 'knowledge'),
    tags: candidate.tags || [],
    triggers: candidate.triggers || [],
    source,
  })
...
export async function extractAndPersistTurnMemory(ctx: ExtensionCtxLike, messages: any[], opts: { sessionFile?: string, trigger?: string } = {}) {
  ...
  const compiled = await executeMemoryTool({ action: 'compile', query: transcript })
  ...
  for (const candidate of candidates) {
    saved.push(await saveCandidate(candidate, opts.trigger || 'extension:llm-extractor'))
  }
