...
async function rebuildRelationGraph(rootDir: string, docs?: MemoryDoc[]) {
  ...
  for (let index = 0; index < allDocs.length; index += 1) {
    for (let inner = index + 1; inner < allDocs.length; inner += 1) {
      ...
      edges.push({
        ...
        reason: relation.reason || "related",
      });
      edges.push({
        from: right.id,
        to: left.id,
  ...
  const graph: MemoryRelationGraph = {
    updated_at: nowIso(),
    edges: edges.sort(
      (a, b) =>
        b.score - a.score ||
...
async function loadRelationGraph(
  rootDir: string,
): Promise<MemoryRelationGraph> {
  return await readJson<MemoryRelationGraph>(relationsPath(rootDir), {
    updated_at: "",
    edges: [],
  });
}
...
async function listEvents(params: Record<string, any> = {}, rootOverride = "") {
  ...
  const results = await loadEvents(root, {
    since: safeString(params.since || ""),
    limit,
  });
...
async function searchEvents(
  ...
  const rows = (await loadEvents(root, { limit: 2_000 }))
    ...
    .sort(
      (a, b) =>
        b.score - a.score ||
        safeString(b.event.created_at).localeCompare(
          safeString(a.event.created_at),
        ),
    )
    .slice(0, limit);
  return {
...
export async function processPendingEvents(
  ...
  const root = resolveMemoryRoot(rootOverride);
  await ensureMemoryLayout(root);
  const state = await readJson<{ lastProcessedAt: string }>(statePath(root), {
    lastProcessedAt: "",
  });
  const pending = await loadEvents(root, {
    since: safeString(params.since || state.lastProcessedAt || ""),
  });
  ...
  const lastProcessedAt = pending.length
    ? pending[pending.length - 1].created_at
...
export async function searchMemories(
  ...
  const topIds = new Set(
    results.slice(0, Math.min(3, results.length)).map((row) => row.doc.id),
  );
  const related = (() => {
    ...
    for (const edge of graph.edges.sort((a, b) => b.score - a.score)) {
      ...
      if (
        docs.some((item) => item.tags.includes(EPISODE_TAG)) &&
  ...
  const events =
    (await searchEvents(query, { limit: Math.min(limit, 8) }, rootOverride))
      .results || [];
  return {
    query,
...
export async function moveMemory(
  ...
  const doc = await resolveMemoryDoc(root, target);
  if (!doc) throw new Error(`memory_not_found:${target}`);
  const nextExposure = ensureExposure(
    safeString(params.exposure || doc.exposure),
    doc.exposure,
  );
  const moved: MemoryDoc = {
    ...
    resident_slot:
      nextExposure === "resident"
        ? safeString(params.residentSlot || doc.resident_slot).trim()
        : "",
...
export function compileMemorySync(
  ...
  try {
    graph = JSON.parse(
      fssync.readFileSync(relationsPath(root), "utf8"),
    ) as MemoryRelationGraph;
  } catch {}
  return compileFromDocsAndEvents(docs, events, graph, params, root);
}
...
export async function doctorMemory(rootOverride = "") {
  ...
  return {
    ...
    chronicle_docs: docs.filter((doc) => doc.tags.includes(CHRONICLE_TAG))
      .length,
...
export async function executeMemoryAction(
  ...
  if (action === "process")
    return await processPendingEvents(params, rootOverride);
