...
export function formatMemoryResult(action: string, response: any): string {
	...
	if (action === 'search') {
		...
		parts.push(rows.length
			? [
				...
				...rows.map((item: any, index: number) => {
					...
					const meta = [`score=${Number(item?.score || 0).toFixed(2)}`, String(item?.exposure || '').trim(), String(item?.scope || '').trim()].filter(Boolean).join(' • ')
		...
		if (related.length) parts.push(['Related memory edges:', ...related.map((item: any, index: number) => `${index + 1}. ${String(item?.title || item?.id || '')} (${String(item?.reason || 'related')})`)].join('\n'))
		if (events.length) parts.push(['Relevant event ledger entries:', ...events.map((item: any, index: number) => `${index + 1}. [${String(item?.created_at || '').replace('T', ' ').slice(0, 16)}] ${String(item?.summary || '')}`)].join('\n'))
		return parts.join('\n\n')
	}
	if (action === 'get') {
		const meta = [
	...
	if (action === 'doctor') {
		return [
			...
			Array.isArray(response?.resident_missing_slots) && response.resident_missing_slots.length ? `- missing resident slots: ${response.resident_missing_slots.join(', ')}` : '- missing resident slots: none',
		].filter(Boolean).join('\n')
	}
	...
	if (action === 'process') {
		const lines = ['Memory processing finished.', response?.status ? `- status: ${String(response.status)}` : '', response?.sessionFile ? `- session file: ${String(response.sessionFile)}` : '', response?.lastProcessedAt ? `- last processed at: ${String(response.lastProcessedAt)}` : '']
		const counts = response?.counts && typeof response.counts === 'object' ? Object.entries(response.counts).map(([key, value]) => `${key}=${String(value)}`).join(', ') : ''
...
export function formatMemoryAgentResult(action: string, response: any): string {
	if (action === 'list') {
		...
		return [`memory list ${rows.length}`,...rows.map((item: any, index: number) => [
	...
	if (action === 'search') {
		...
		const events = Array.isArray(response?.event_matches) ? response.event_matches : []
		const parts: string[] = []
		parts.push(rows.length ? [`memory search ${String(response?.query || '')} (${rows.length})`, ...rows.map((item: any, index: number) => [`${index + 1}. ${String(item?.title || item?.id || '(untitled)')}`, `score=${Number(item?.score || 0).toFixed(2)}`, String(item?.exposure || '').trim(), String(item?.scope || '').trim(), `path=${String(item?.path || '')}`].filter(Boolean).join(' | '))].join('\n') : `memory search ${String(response?.query || '')} (0)`)
	...
	if (action === 'get') {
		return [String(response?.title || response?.id || 'Memory document'), `id=${String(response?.id || '')}`, `exposure=${String(response?.exposure || '')}`, `scope=${String(response?.scope || '')}`, `kind=${String(response?.kind || '')}`, response?.resident_slot ? `slot=${String(response.resident_slot)}` : '', `path=${String(response?.path || '')}`, 'read file separately if full body is needed'].filter(Boolean).join('\n')
	}
	...
	if (action === 'compile') {
		const sections = [['resident_docs', response?.resident_docs], ['progressive_docs', response?.progressive_docs], ['expanded_progressives', response?.expanded_progressives], ['episode_docs', response?.episode_docs], ['recall_docs', response?.recall_docs], ['related_docs', response?.related_docs]].filter(([, value]) => Array.isArray(value) && value.length > 0) as Array<[string, any[]]>
		if (!sections.length) return 'memory compile 0 sources'
		return [`memory compile ${String(response?.query || '').trim() || '(no query)'}`, ...sections.map(([name, docs]) => `${name}: ${docs.length}`), ...sections.flatMap(([name, docs]) => docs.map((doc: any, index: number) => `${name}[${index + 1}] path=${String(doc?.path || '')}`))].join('\n')
	}
	if (action === 'doctor') return ['memory doctor', `root=${String(response?.root || '')}`, `total=${String(response?.total || 0)}`, `active=${String(response?.active_total || 0)}`, `events=${String(response?.event_count || 0)}`, `relation_edges=${String(response?.relation_edges || 0)}`, Array.isArray(response?.resident_missing_slots) ? `missing_slots=${response.resident_missing_slots.join(',')}` : ''].filter(Boolean).join('\n')
	...
	if (action === 'events' || action === 'event_search') {
		...
		return [action === 'event_search' ? `memory event_search ${String(response?.query || '')} (${rows.length})` : `memory events ${rows.length}`, ...rows.map((item: any, index: number) => `${index + 1}. ${String(item?.created_at || '')} | ${String(item?.summary || '')}`)].join('\n')
	}
	if (action === 'process') return ['memory process', `status=${String(response?.status || response?.ok || 'ok')}`, response?.sessionFile ? `sessionFile=${String(response.sessionFile)}` : '', response?.lastProcessedAt ? `lastProcessedAt=${String(response.lastProcessedAt)}` : ''].filter(Boolean).join('\n')
