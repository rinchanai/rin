...
function readInitState(resolveAgentDir: () => string) {
	try {
		const parsed = JSON.parse(fs.readFileSync(initStatePath(resolveAgentDir), 'utf8')) as Record<string, any>
		return { version: 2, promptedAt: '', completedAt: '', lastTrigger: '', pending: false, ...parsed }
	} catch {
		return { version: 2, promptedAt: '', completedAt: '', lastTrigger: '', pending: false }
	}
}

function writeInitState(resolveAgentDir: () => string, next: Record<string, any>) {
	fs.mkdirSync(path.dirname(initStatePath(resolveAgentDir)), { recursive: true })
	fs.writeFileSync(initStatePath(resolveAgentDir), JSON.stringify(next, null, 2), 'utf8')
}
...
export function isOnboardingActive(resolveAgentDir: () => string, state = readInitState(resolveAgentDir)) {
	return Boolean(state?.pending)
}

export async function getOnboardingStatus(resolveAgentDir: () => string, loadMemoryService: () => Promise<any>) {
...
export async function markOnboardingPrompted(resolveAgentDir: () => string, trigger: string) {
	const state = readInitState(resolveAgentDir)
	const next = { ...state, version: 2, promptedAt: new Date().toISOString(), completedAt: '', lastTrigger: trigger, pending: true }
	writeInitState(resolveAgentDir, next)
	return next
}

export async function refreshOnboardingCompletion(resolveAgentDir: () => string, loadMemoryService: () => Promise<any>) {
	const status = await getOnboardingStatus(resolveAgentDir, loadMemoryService)
	if (status.complete) {
		const next = { ...status.state, version: 2, completedAt: new Date().toISOString(), pending: false }
		writeInitState(resolveAgentDir, next)
