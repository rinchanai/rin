...
export type MemoryPromptAction = "add" | "replace" | "remove";
...
function applySlotAction(options: {
  ...
  if (action === "add") {
    if (!incomingContent) throw new Error("memory_prompt_content_required:add");
    return [existingContent, incomingContent].filter(Boolean).join("\n");
  }

  if (!oldText) throw new Error(`memory_prompt_old_text_required:${action}`);
  if (!existingContent) throw new Error(`memory_prompt_slot_empty:${action}`);

  const matchCount = countSubstringMatches(existingContent, oldText);
  if (matchCount === 0)
    throw new Error(`memory_prompt_old_text_not_found:${action}`);
  if (matchCount > 1)
    throw new Error(`memory_prompt_old_text_ambiguous:${action}`);

  if (action === "replace") {
    if (!incomingContent)
      throw new Error("memory_prompt_content_required:replace");
...
export async function refineMemoryPromptSlot(options: {
  ...
  const content = applySlotAction({
    action: (options.action || "add") as MemoryPromptAction,
