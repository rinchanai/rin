...
function extractFilePaths(value: string): string[] {
  const matches = safeString(value).match(/(?:[A-Za-z]:)?[~./\w-]+\.[A-Za-z0-9_-]{1,12}/g) || []
  return uniqueStrings(matches.map((item) => item.trim()).filter((item) => /[/.]/.test(item)))
}
...
function repoNameFromCwd(cwd: string): string {
  const base = path.basename(safeString(cwd).trim())
  return base || 'project'
}
...
function shouldCaptureExplicitMemory(text: string): boolean {
  const raw = normalizeMessageText(text)
  if (!raw || raw.length < 8) return false
  if (/(记住|以后|始终|总是|默认|通常|平时|我喜欢|我不喜欢|请用|请先|不要|别|prefer|always|default|remember|from now on|in general)/i.test(raw)) return true
  if (chooseResidentSlot(raw)) return true
  if (/(代码|改代码|修改代码|实现|patch|edit|refactor|副作用|小步|保守|尽量)/i.test(raw)) return true
  return false
}

function isReplacementStyleCue(text: string): boolean {
  return /(改成|改为|以后都按|以后统一|从现在开始|今后统一|不要再|不再|不是.*而是|记错|纠正一下|更新一下|改口|以这个为准)/i.test(text)
}
...
function mergeBulletContent(existing: string, next: string, maxChars: number): string {
  const bullets = uniqueStrings([...splitBullets(existing), ...splitBullets(next)])
  const rendered = bullets.map((item) => `- ${item}`).join('\n')
  if (rendered.length <= maxChars) return rendered
  const kept: string[] = []
  for (const bullet of bullets) {
    const candidate = [...kept, bullet].map((item) => `- ${item}`).join('\n')
...
function chooseResidentSlot(text: string): string | null {
  const raw = normalizeNeedle(text)
  if (!raw) return null
  if (/(说话|语气|风格|自然|机械|简洁|啰嗦|聊天|口吻)/i.test(text)) return 'core_voice_style'
  if (/(先.*设计.*再.*实现|先讨论.*再.*实现|方法论|原则|边界|做事方式|思考方式|methodology|principle)/i.test(text)) return 'core_methodology'
  if (/(价值观|世界观|底线|重要的是|应该追求|value|worldview)/i.test(text)) return 'core_values'
  if (/(你是|你的身份|扮演|人设|身份设定)/i.test(text)) return 'agent_identity'
  if (/(我是|用户是|主人|我的身份|关于我)/i.test(text)) return 'owner_identity'
  return null
}

function classifyScope(text: string, cwd: string): MemoryScope {
  if (chooseResidentSlot(text)) return 'global'
  if (/(这个项目|当前项目|这个仓库|当前仓库|repo|repository|项目里|仓库里)/i.test(text)) return 'project'
  const repoName = repoNameFromCwd(cwd)
  if (repoName && new RegExp(`\\b${repoName.replace(/[-/\\^$*+?.()|[\]{}]/g, '\\$&')}\\b`, 'i').test(text)) return 'project'
  if (/(代码|改代码|修改代码|实现|patch|edit|refactor|bash|tool|命令|文件改动)/i.test(text)) return 'domain'
  return 'global'
}

function candidateTitleForProgressive(text: string): string {
  if (/(代码|改代码|修改代码|实现|patch|edit|refactor|副作用|小步|保守)/i.test(text)) return 'Code modification preferences'
  if (/(规章|制度|规则|workflow|流程|步骤)/i.test(text)) return 'Working rules and procedures'
  return 'Long-form working preferences'
}

function buildProgressiveTriggers(text: string): string[] {
  const triggers = ['code', 'edit', 'patch', 'implement', 'modify']
  if (/(代码|改代码|修改代码|实现|补丁|重构|副作用|小步|保守)/i.test(text)) triggers.push('代码', '修改', '实现', '补丁', '重构', '副作用', '小步', '保守')
  if (/(规章|制度|规则|流程)/i.test(text)) triggers.push('规则', '流程', '制度')
  return uniqueStrings(triggers)
}

function buildRecallTitle(text: string, cwd: string): string {
  const repoName = repoNameFromCwd(cwd)
  if (/(extension|core)/i.test(text)) return `${repoName}: extension and core boundary context`
  if (/(koishi)/i.test(text)) return `${repoName}: koishi context`
  if (/(memory)/i.test(text)) return `${repoName}: memory design context`
  return `${repoName}: working context`
}
...
function summarizeCounts(events: MemoryEvent[]) {
  return {
    total: events.length,
    user: events.filter((item) => item.kind === 'user_input').length,
    assistant: events.filter((item) => item.kind === 'assistant_message').length,
    tools: events.filter((item) => item.kind === 'tool_result').length,
    errors: events.filter((item) => item.kind === 'tool_result' && item.is_error).length,
  }
}

function eventToEpisodeLine(event: MemoryEvent): string {
  const timestamp = safeString(event.created_at).slice(11, 16) || '??:??'
  return `- [${timestamp}] ${event.summary}`
}

function inferEpisodePreferences(events: MemoryEvent[]): string[] {
  return uniqueStrings(events
    .filter((item) => item.kind === 'user_input')
    .map((item) => normalizeMessageText(item.text))
    .filter((text) => /(以后|默认|通常|平时|喜欢|不喜欢|请用|请先|不要|别|尽量|风格|自然|机械|保守|小步|副作用|先讨论|先设计)/i.test(text))
    .map((text) => trimText(text, 120)))
}

function inferEpisodeDecisions(events: MemoryEvent[]): string[] {
  return uniqueStrings(events
    .filter((item) => item.kind !== 'tool_result')
    .flatMap((item) => splitBullets(item.text))
    .filter((text) => /(不要|优先|应该|决定|改成|改为|采用|保持|先.*再.*|extension|core|方案|边界)/i.test(text))
    .map((text) => trimText(text, 120)))
}

function inferEpisodeOpenThreads(events: MemoryEvent[]): string[] {
  return uniqueStrings(events
    .flatMap((item) => splitBullets(item.text))
    .filter((text) => /(?:\?|？|TODO|todo|接下来|下一步|后面|待做|还要|还没|需要继续|需要确认|需要讨论|研究一下|follow up)/i.test(text))
    .map((text) => trimText(text, 120)))
}

function inferEpisodeTools(events: MemoryEvent[]): string[] {
  return uniqueStrings(events
    .filter((item) => item.kind === 'tool_result')
    .map((item) => `${item.tool_name || 'tool'}${item.is_error ? ' (error)' : ''}`)
    .filter(Boolean))
}

function inferEpisodeFiles(events: MemoryEvent[]): string[] {
  return uniqueStrings(events.flatMap((item) => extractFilePaths(item.text))).slice(0, 20)
}

function inferEpisodeTriggers(events: MemoryEvent[]): string[] {
  return uniqueStrings(events.flatMap((item) => conceptTokens(`${item.summary}\n${item.text}`))).slice(0, 32)
}

function episodeSummaryFromEvents(events: MemoryEvent[], session: string, date: string): string {
  const counts = summarizeCounts(events)
  const decisions = inferEpisodeDecisions(events)
  const preferences = inferEpisodePreferences(events)
  const leading = decisions[0] || preferences[0] || 'Ongoing collaborative work and memory-relevant activity.'
  return trimText(`${leading} Episode for ${session} on ${date}; ${counts.user} user turns, ${counts.assistant} assistant turns, ${counts.tools} tool results${counts.errors ? `, ${counts.errors} tool errors` : ''}.`, 220)
}

function renderEpisodeContent(events: MemoryEvent[], session: string, date: string): string {
  const counts = summarizeCounts(events)
  const preferences = inferEpisodePreferences(events)
  const decisions = inferEpisodeDecisions(events)
  const openThreads = inferEpisodeOpenThreads(events)
  const tools = inferEpisodeTools(events)
  const files = inferEpisodeFiles(events)
  const sections = [
    `Session episode synthesized from the event ledger for ${session} on ${date}.`,
    '',
    '## Summary',
    `- ${counts.user} user turns`,
    `- ${counts.assistant} assistant turns`,
    `- ${counts.tools} tool results${counts.errors ? ` (${counts.errors} errors)` : ''}`,
  ]
  if (preferences.length) sections.push('', '## Emerging preferences', ...preferences.map((item) => `- ${item}`))
  if (decisions.length) sections.push('', '## Decisions and constraints', ...decisions.map((item) => `- ${item}`))
  if (tools.length) sections.push('', '## Tool activity', ...tools.map((item) => `- ${item}`))
  if (files.length) sections.push('', '## Files mentioned', ...files.map((item) => `- ${item}`))
  if (openThreads.length) sections.push('', '## Open threads', ...openThreads.map((item) => `- ${item}`))
  sections.push('', '## Timeline', ...events.map(eventToEpisodeLine))
...
async function appendChronicleEntry(rootDir: string, event: MemoryEvent) {
  ...
  const line = eventToEpisodeLine(event)
...
async function rebuildEpisodeDocs(rootDir: string, events: MemoryEvent[]) {
  ...
  for (const [key, group] of grouped) {
    ...
    const id = slugify(`${date}-${session}-episode`, `${date}-episode`)
    const filePath = genericDocPath(rootDir, 'recall', id, 'episodes')
    const content = renderEpisodeContent(group, session, date)
    const repoTag = uniqueStrings(group.map((item) => repoNameFromCwd(item.cwd)).filter(Boolean))
    const next = normalizeFrontmatter({
      id,
      title: `${date} ${session} episode`,
      exposure: 'recall',
      fidelity: 'fuzzy',
      summary: episodeSummaryFromEvents(group, session, date),
      tags: uniqueStrings([EPISODE_TAG, session, ...repoTag]),
      triggers: uniqueStrings(['episode', 'summary', 'history', 'timeline', ...inferEpisodeTriggers(group)]),
      scope: 'session',
      kind: 'history',
      source: 'memory:episode-builder',
    }, filePath, content)
    const existing = fssync.existsSync(filePath) ? parseMarkdownDoc(filePath, await fs.readFile(filePath, 'utf8')) : null
    if (existing && existing.content === next.content && existing.summary === next.summary) continue
...
async function upsertResidentSlot(rootDir: string, slot: string, line: string, summary: string, source: string, options: { replace?: boolean } = {}) {
  const filePath = residentPath(rootDir, slot)
  const existing = fssync.existsSync(filePath)
    ...
    : normalizeFrontmatter({
      ...
      summary,
      scope: 'global',
      kind: slot === 'core_voice_style' ? 'style' : slot === 'core_methodology' ? 'method' : slot === 'core_values' ? 'value' : 'preference',
      source,
      canonical: true,
    }, filePath, '')
  const limits = RESIDENT_LIMITS[slot]
  existing.path = filePath
  existing.exposure = 'resident'
  existing.fidelity = existing.fidelity || 'fuzzy'
  existing.resident_slot = slot
  existing.scope = 'global'
  existing.canonical = true
  existing.summary = summary || existing.summary
  existing.source = source || existing.source
  existing.updated_at = nowIso()
  existing.last_observed_at = existing.updated_at
  existing.observation_count = Math.max(1, Number(existing.observation_count || 1) + 1)
  existing.status = 'active'
  existing.content = options.replace ? mergeBulletContent('', line, limits.maxChars) : mergeBulletContent(existing.content, line, limits.maxChars)
  assertResidentDoc(existing)
  await writeMemoryDoc(existing)
  return existing
}

async function upsertByTitle(rootDir: string, exposure: MemoryExposure, title: string, patch: Partial<MemoryDoc> & { content: string, replace?: boolean }, subgroup = '') {
  const docs = await loadMemoryDocs(rootDir)
  const target = docs.find((doc) => doc.exposure === exposure && normalizeNeedle(doc.title) === normalizeNeedle(title))
  const id = target?.id || slugify(title, `memory-${sha(title).slice(0, 8)}`)
  const filePath = target?.path || genericDocPath(rootDir, exposure, id, subgroup)
  const base = target || normalizeFrontmatter({
    id,
    title,
    exposure,
    fidelity: patch.fidelity || 'fuzzy',
  ...
  const doc: MemoryDoc = {
    ...
    observation_count: Math.max(1, Number(base.observation_count || 1) + 1),
    status: 'active',
    tags: uniqueStrings([...(base.tags || []), ...(patch.tags || [])]),
    aliases: uniqueStrings([...(base.aliases || []), ...(patch.aliases || [])]),
    triggers: uniqueStrings([...(base.triggers || []), ...(patch.triggers || [])]),
    supersedes: uniqueStrings([...(base.supersedes || []), ...(patch.supersedes || [])]),
    summary: safeString(patch.summary || base.summary).trim(),
    content: patch.replace ? safeString(patch.content || base.content).trim() : safeString(patch.content || base.content).trim(),
    fidelity: ensureFidelity(safeString(patch.fidelity || base.fidelity || 'fuzzy')),
    scope: ensureScope(safeString(patch.scope || base.scope || 'project')),
    kind: ensureKind(safeString(patch.kind || base.kind || 'knowledge')),
    canonical: exposure === 'resident' ? true : Boolean(patch.canonical ?? base.canonical),
  }
  await writeMemoryDoc(doc)
  return doc
}

function buildExtractionCandidates(event: MemoryEvent) {
  const text = normalizeMessageText(event.text)
  if (!shouldCaptureExplicitMemory(text)) return [] as Array<Record<string, any>>
  const scope = classifyScope(text, event.cwd)
  const residentSlot = chooseResidentSlot(text)
  const replace = isReplacementStyleCue(text)
  if (residentSlot && scope === 'global') {
    return [{
      exposure: 'resident',
      residentSlot,
      line: trimText(text, 220),
      summary: residentSlot === 'core_voice_style'
        ? 'Global speaking style and tone baseline.'
        : residentSlot === 'core_methodology'
          ? 'Global working methodology baseline.'
          : residentSlot === 'core_values'
            ? 'Global value and worldview baseline.'
            : residentSlot === 'owner_identity'
              ? 'Core owner identity facts.'
              : 'Core agent identity facts.',
      replace,
      source: `auto:${event.id}`,
    }]
  }
  if (scope === 'domain') {
    return [{
      exposure: 'progressive',
      title: candidateTitleForProgressive(text),
      summary: trimText(text, 140),
      content: mergeBulletContent('', text, 2_000),
      scope: 'domain',
      kind: 'preference',
      triggers: buildProgressiveTriggers(text),
      tags: ['auto', 'working-preference'],
      replace,
      source: `auto:${event.id}`,
    }]
  }
  return [{
    exposure: 'recall',
    title: buildRecallTitle(text, event.cwd),
    summary: trimText(text, 140),
    content: [
      `Project-specific memory from user input on ${event.created_at}.`,
      '',
      `- ${text}`,
    ].join('\n'),
    scope: scope === 'global' ? 'project' : scope,
    kind: 'knowledge',
    triggers: normalizeList([repoNameFromCwd(event.cwd), 'project context']),
    tags: ['auto', 'project-context', repoNameFromCwd(event.cwd)],
    replace,
    source: `auto:${event.id}`,
  }]
}

async function applyExtractionCandidate(rootDir: string, candidate: Record<string, any>) {
  if (candidate.exposure === 'resident') {
    const doc = await upsertResidentSlot(rootDir, safeString(candidate.residentSlot), safeString(candidate.line), safeString(candidate.summary), safeString(candidate.source), { replace: Boolean(candidate.replace) })
    return { exposure: 'resident', doc: previewMemoryDoc(doc) }
  }
  if (candidate.exposure === 'progressive') {
    const existing = (await loadMemoryDocs(rootDir)).find((doc) => doc.exposure === 'progressive' && normalizeNeedle(doc.title) === normalizeNeedle(safeString(candidate.title)))
    const nextContent = existing ? mergeBulletContent(existing.content, safeString(candidate.content), 8_000) : safeString(candidate.content)
    const doc = await upsertByTitle(rootDir, 'progressive', safeString(candidate.title), {
      summary: safeString(candidate.summary),
      content: nextContent,
      scope: ensureScope(safeString(candidate.scope || 'domain')),
      kind: ensureKind(safeString(candidate.kind || 'preference')),
      triggers: normalizeList(candidate.triggers || []),
      tags: normalizeList(candidate.tags || []),
      source: safeString(candidate.source || ''),
      replace: Boolean(candidate.replace),
    })
    return { exposure: 'progressive', doc: previewMemoryDoc(doc) }
  }
  const existing = (await loadMemoryDocs(rootDir)).find((doc) => doc.exposure === 'recall' && normalizeNeedle(doc.title) === normalizeNeedle(safeString(candidate.title)))
  const nextContent = existing ? mergeBulletContent(existing.content, safeString(candidate.summary || candidate.content), 16_000) : safeString(candidate.content)
  const doc = await upsertByTitle(rootDir, 'recall', safeString(candidate.title), {
    summary: safeString(candidate.summary),
    content: nextContent,
    scope: ensureScope(safeString(candidate.scope || 'project')),
    kind: ensureKind(safeString(candidate.kind || 'knowledge')),
    triggers: normalizeList(candidate.triggers || []),
    tags: normalizeList(candidate.tags || []),
    source: safeString(candidate.source || ''),
    replace: Boolean(candidate.replace),
  })
  return { exposure: 'recall', doc: previewMemoryDoc(doc) }
}

async function reconcileMemoryLifecycle(rootDir: string) {
  const docs = activeDocsOnly(await loadMemoryDocs(rootDir))
  const changes: Array<Record<string, any>> = []

  for (const doc of docs) {
    if (doc.exposure === 'recall' && doc.scope === 'domain' && doc.kind === 'preference' && Number(doc.observation_count || 0) >= 2) {
      const moved = {
        ...doc,
        exposure: 'progressive' as MemoryExposure,
        path: genericDocPath(rootDir, 'progressive', doc.id),
        updated_at: nowIso(),
      }
      await fs.rm(doc.path, { force: true })
      await writeMemoryDoc(moved)
      changes.push({ action: 'promote', from: 'recall', to: 'progressive', id: doc.id })
    }
  }

  const refreshed = activeDocsOnly(await loadMemoryDocs(rootDir))
  for (const doc of refreshed) {
    if (doc.exposure !== 'progressive') continue
    if (doc.scope !== 'global') continue
    if (Number(doc.observation_count || 0) < 3) continue
    const slot = chooseResidentSlot(`${doc.title}\n${doc.summary}\n${doc.content}`)
    if (!slot) continue
    const limits = RESIDENT_LIMITS[slot]
    const compact = mergeBulletContent('', `${doc.summary}\n${doc.content}`, limits.maxChars)
    const resident = await upsertResidentSlot(rootDir, slot, compact, doc.summary || `${slot} promoted from progressive memory.`, `promoted:${doc.id}`, { replace: false })
    const superseded: MemoryDoc = { ...doc, status: 'superseded', supersedes: uniqueStrings([...doc.supersedes, resident.id]), updated_at: nowIso() }
    await writeMemoryDoc(superseded)
    changes.push({ action: 'promote', from: 'progressive', to: 'resident', id: doc.id, slot })
  }

  const after = await loadMemoryDocs(rootDir)
  await rebuildRelationGraph(rootDir, after)
  return changes
}
...
export async function processPendingEvents(params: Record<string, any> = {}, rootOverride = '') {
  ...
  for (const event of pending) {
    ...
    for (const candidate of buildExtractionCandidates(event)) {
      applied.push(await applyExtractionCandidate(root, candidate))
    }
  }
  const allEvents = await loadEvents(root, { limit: 10_000 })
  const episodeDocsUpdated = await rebuildEpisodeDocs(root, allEvents)
