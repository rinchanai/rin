...
function wrapAgentPrompt(
  ...
  if (!body) return body;
  if (body.includes("[Scheduled task managed by manage_scheduled_tasks]"))
    return body;
  return [
...
export default function cronExtension(pi: ExtensionAPI) {
  pi.registerTool({
    ...
    description:
      "Create, inspect, update, pause, resume, complete, and delete daemon scheduled tasks.",
    promptSnippet:
      "Manage daemon scheduled tasks for recurring, one-time, or interval-based automation.",
    promptGuidelines: [
    ...
    parameters: Type.Object({
      ...
      taskId: Type.Optional(
        Type.String({
          description:
            "Required for get/delete/pause/resume/complete, and for updating an existing task via save.",
        }),
      ),
      reason: Type.Optional(
        Type.String({
          description: "Optional completion reason for action=complete.",
        }),
      ),
      task: Type.Optional(
        Type.Object({
          ...
          enabled: Type.Optional(
            Type.Boolean({
              description:
                "Whether the task should remain enabled after saving.",
            }),
          ),
          ...
          trigger: Type.Optional(
            Type.Object({
              ...
              startAt: Type.Optional(
                Type.String({
                  description:
                    "Optional ISO timestamp for the first interval run.",
                }),
              ),
            ...
            Type.Union([
              Type.Object({
                ...
                stopAt: Type.Optional(
                  Type.String({
                    description:
                      "ISO timestamp after which the task should stop.",
                  }),
                ),
              }),
              Type.Null(),
          ...
          target: Type.Optional(
            Type.Object({
              ...
              command: Type.Optional(
                Type.String({
                  description: "Shell command for direct execution.",
                }),
              ),
              shell: Type.Optional(
                Type.String({
                  description: "Optional shell path for shell_command.",
                }),
              ),
            }),
          ),
