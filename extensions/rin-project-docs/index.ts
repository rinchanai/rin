...
function buildPromptBlock() {
  ...
  const piReadme =
    piDocs.find((item) => item.endsWith("/README.md")) || piDocs[0];
  const piDocsDir =
    piDocs
      .find((item) => item.includes("/docs/extensions.md"))
      ?.replace(/\/extensions\.md$/, "") || piDocs[0];
  const piExamplesDir =
    piDocs
      .find((item) => item.endsWith("/examples/README.md"))
      ?.replace(/\/README\.md$/, "") || piDocs[0];
  const rinReadme =
    rinDocs.find((item) => item.endsWith("/README.md")) || rinDocs[0];
  const rinRuntimeLayout =
    rinDocs.find((item) => item.endsWith("/runtime-layout.md")) || rinDocs[0];
  const rinBuiltinExtensions =
    rinDocs.find((item) => item.endsWith("/builtin-extensions.md")) ||
    rinDocs[0];
  const rinCapabilities =
    rinDocs.find((item) => item.endsWith("/capabilities.md")) || rinDocs[0];
