...
export async function setRpcModel(target: any, model: any, refreshModels: () => Promise<any>) {
  ...
  await target.call('set_model', { provider: model.provider, modelId: model.id })
  await refreshModels()
}

export async function cycleRpcModel(target: any, direction: 'forward' | 'backward' | undefined, getAvailableModels: () => any[], refreshModels: () => Promise<any>) {
  if (target.detachedBlankSession) {
    const available = target.scopedModels.length > 0 ? target.scopedModels.map((entry: any) => entry.model) : getAvailableModels()
    if (available.length <= 1) return undefined
    const step = direction === 'backward' ? -1 : 1
    const currentIndex = Math.max(0, available.findIndex((model: any) => model?.provider === target.model?.provider && model?.id === target.model?.id))
...
export function setRpcThinkingLevel(target: any, level: ThinkingLevel) {
  ...
  void target.client.send({ type: 'set_thinking_level', level: next }).catch(() => {})
}
...
export function cycleRpcThinkingLevel(target: any): ThinkingLevel | undefined {
  ...
  const next = levels[(Math.max(0, levels.indexOf(target.thinkingLevel)) + 1) % levels.length]
...
export function setRpcAutoCompaction(target: any, enabled: boolean) {
  ...
  void target.client.send({ type: 'set_auto_compaction', enabled }).catch(() => {})
}
