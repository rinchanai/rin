...
export function targetUserRuntimeEnv(targetUser: string, env: Record<string, string> = {}) {
  const target = readPasswdUser(targetUser)
...
export function buildUserShell(targetUser: string, argv: string[], env: Record<string, string> = {}) {
  const currentUser = os.userInfo().username
