...
export function buildKoishiConfigFromSettings(settings: any) {
  ...
  applyAdapterPlugins(config.plugins, 'adapter-onebot', koishi?.onebot, {
    protocol: 'ws',
    endpoint: '',
    selfId: '',
    token: '',
  }, 'onebot')
  applyAdapterPlugins(config.plugins, 'adapter-telegram', koishi?.telegram, {
    protocol: 'polling',
    token: '',
    slash: true,
  }, 'telegram')
    ...
    for (const platform of platforms) {
      ...
      for (const first of levelOne) {
        ...
        if (fs.existsSync(directStatePath)) {
          out.push({ chatKey: composeChatKey(platform, first), statePath: directStatePath })
          continue
        }
        ...
        for (const chatId of levelTwo) {
          ...
          if (fs.existsSync(statePath)) out.push({ chatKey: composeChatKey(platform, chatId, first), statePath })
        }
      }
    }
  } catch {}
...
export function ensureIdentitySeed(dataDir: string) {
  ...
  writeJsonFile(filePath, { persons: { owner: { trust: 'OWNER' } }, aliases: [], trusted: [] })
}

export function loadIdentity(dataDir: string) {
  ensureIdentitySeed(dataDir)
  const identity = readJsonFile<any>(identityPath(dataDir), { persons: {}, aliases: [], trusted: [] })
  identity.persons ||= {}
  identity.aliases ||= []
  identity.trusted ||= []
  return identity
...
export function trustOf(identity: any, platform: string, userId: string) {
  ...
  const alias = (Array.isArray(identity?.aliases) ? identity.aliases : []).find((entry: any) => entry && entry.platform === nextPlatform && String(entry.userId) === nextUserId)
...
export function canAccessAgentInput({ chatType, trust, mentionLike = false, commandLike = false }: { chatType: 'private' | 'group'; trust: string; mentionLike?: boolean; commandLike?: boolean }) {
...
export function findBot(app: any, platform: string, botId = '') {
  ...
  return matches.find((bot: any) => safeString(bot?.selfId).trim() === nextBotId) || null
}

export function ensureFileName(name: string, fallback = 'attachment') {
  const base = safeString(name).trim().replace(/[\\/:*?"<>|]+/g, '_').replace(/^\.+$/, '')
