...
export function readInstallerJson<T>(filePath: string, fallback: T, elevated = false): T {
  try {
    return JSON.parse(fs.readFileSync(filePath, 'utf8')) as T
  } catch (error: any) {
...
export function writeLaunchersForUser(userName: string, installDir: string, homeForUser: (user: string) => string) {
  ...
  return { rinPath: path.join(binDir, 'rin'), rinInstallPath: path.join(binDir, 'rin-install') }
}

export function appConfigDirForUser(userName: string, homeForUser: (user: string) => string) {
...
export function runCommandAsUser(targetUser: string, command: string, args: string[], extraEnv: Record<string, string> = {}) {
...
export function writeTextFileWithPrivilege(filePath: string, value: string, ownerUser?: string, ownerGroup?: string | number, mode = 0o600) {
...
export function writeJsonFileWithPrivilege(filePath: string, value: unknown, ownerUser?: string, ownerGroup?: string | number) {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'rin-install-write-'))
...
export function syncInstalledDocTree(sourceDir: string, destDir: string, targetUser: string, elevated = false, deps: { findSystemUser: (user: string) => any }) {
  ...
  if (elevated) {
    ...
    if (target?.name) runPrivileged('chown', ['-R', `${target.name}${targetGroup ? `:${targetGroup}` : ''}`, destDir])
    return destDir
  }
...
export function syncInstalledDocs(sourceRoot: string, installDir: string, targetUser: string, elevated = false, deps: { findSystemUser: (user: string) => any }) {
  const installedRinDocsDir = syncInstalledDocTree(path.join(sourceRoot, 'docs', 'rin'), path.join(installDir, 'docs', 'rin'), targetUser, elevated, deps)
  ...
  for (const name of ['README.md', 'CHANGELOG.md', 'docs', 'examples']) {
    const synced = syncInstalledDocTree(path.join(piDocRoot, name), path.join(piInstallRoot, name), targetUser, elevated, deps)
...
export function publishInstalledRuntime(sourceRoot: string, installDir: string, targetUser: string, elevated = false, deps: { findSystemUser: (user: string) => any }) {
  ...
  for (const name of ['dist', 'node_modules', 'third_party', 'extensions', 'package.json']) syncTree(path.join(sourceRoot, name), path.join(releaseRoot, name))
...
export function listInstalledReleaseNames(installDir: string, elevated = false) {
  const releasesDir = path.join(installDir, 'app', 'releases')
  if (!elevated) {
    try {
      return fs.readdirSync(releasesDir, { withFileTypes: true }).filter((entry) => entry.isDirectory()).map((entry) => entry.name)
    } catch {
      return [] as string[]
    }
  }
  const privilegeCommand = pickPrivilegeCommand()
  try {
    const raw = execFileSync(privilegeCommand, [process.execPath, '-e', `const fs=require('node:fs');const dir=process.argv[1];try{const names=fs.readdirSync(dir,{withFileTypes:true}).filter((entry)=>entry.isDirectory()).map((entry)=>entry.name);process.stdout.write(JSON.stringify(names));}catch{process.stdout.write('[]')}`, releasesDir], { encoding: 'utf8' })
...
export function pruneInstalledReleases(installDir: string, keepCount: number, currentReleaseRoot: string, elevated = false) {
  const releasesDir = path.join(installDir, 'app', 'releases')
  const currentReleaseName = path.basename(currentReleaseRoot)
  const names = listInstalledReleaseNames(installDir, elevated).sort((a, b) => b.localeCompare(a))
