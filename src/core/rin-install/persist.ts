...
export function reconcileInstallerManifest(options: {
  targetUser: string
  installDir: string
  provider?: string
  modelId?: string
  ...
  readInstallerJson: <T>(filePath: string, fallback: T, elevated?: boolean) => T
  writeJsonFileWithPrivilege: (filePath: string, value: unknown, ownerUser?: string, ownerGroup?: string | number) => void
  ...
  const manifestJson = deps.readInstallerJson<any>(manifestPath, deps.readInstallerJson<any>(legacyManifestPath, {}, Boolean(options.elevated)), Boolean(options.elevated))
  ...
  if (options.elevated) {
    deps.writeJsonFileWithPrivilege(manifestPath, manifestJson, ownerUser, ownerGroup)
    try { deps.runPrivileged('rm', ['-f', legacyManifestPath]) } catch {}
  } else {
...
export async function persistInstallerOutputs(options: {
  currentUser: string
  targetUser: string
  installDir: string
  provider: string
  ...
  const settingsJson = deps.readInstallerJson<any>(settingsPath, {}, Boolean(options.elevated))
  ...
  const authJson = deps.readInstallerJson<any>(authPath, {}, Boolean(options.elevated))
  const nextAuthJson = { ...authJson, ...(options.authData || {}) }

  const launcherPath = path.join(deps.appConfigDirForUser(options.currentUser), 'install.json')
  ...
  if (options.elevated) {
    deps.writeJsonFileWithPrivilege(settingsPath, settingsJson, ownerUser, ownerGroup)
    deps.writeJsonFileWithPrivilege(authPath, nextAuthJson, ownerUser, ownerGroup)
  } else {
  ...
  const launchers = deps.writeLaunchersForUser(options.currentUser, options.installDir)
