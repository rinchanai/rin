...
export async function startUpdater(deps: {
  ...
  const target = targets.length === 1
    ? targets[0]!
    : targets[Number(deps.ensureNotCancelled(await select({
  ...
  const shouldProceed = deps.ensureNotCancelled(await confirm({
  ...
  } satisfies FinalizeInstallOptions, 'Publishing runtime and refreshing the installed target...', {
