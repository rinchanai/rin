...
export function computeAvailableThinkingLevels(model: { provider: string; id: string; reasoning: boolean }) {
...
export async function createInstallerAuthStorage(installDir: string, readJsonFile: <T>(filePath: string, fallback: T) => T) {
...
export async function configureProviderAuth(
  ...
  const authStorage = await createInstallerAuthStorage(installDir, deps.readJsonFile)
    ...
    try {
      await authStorage.login(provider, {
        ...
        async onPrompt(prompt: { message: string; placeholder?: string }) {
          return String(deps.ensureNotCancelled(await text({
  ...
  const token = String(deps.ensureNotCancelled(await text({
