...
import { acquireProcessLock, listInstanceIds as listSidecarInstanceIds, readInstanceState as readSidecarInstanceState, writeInstanceState as writeSidecarInstanceState } from '../sidecar/common.js'
import type { SidecarInstanceState, SidecarStatusRow } from '../sidecar/types.js'
...
function readInstanceState(stateRoot: string, instanceId: string) {
  return readSidecarInstanceState<any>(instanceStateFileForState(stateRoot, instanceId))
}
...
async function ensureKoishiSidecar(stateRoot: string, options: { instanceId?: string; entryPath?: string } = {}) {
  ...
  const release = await acquireProcessLock(lockPathForState(stateRoot)).catch((error: any) => {
    throw new Error(String(error?.message || error || `koishi_lock_timeout:${lockPathForState(stateRoot)}`))
  })
...
function getKoishiSidecarStatus(stateRoot: string) {
  const instances = listInstanceIds(stateRoot).map((instanceId): SidecarStatusRow => {
    const state = (readInstanceState(stateRoot, instanceId) || {}) as SidecarInstanceState
    const pid = Number(state?.pid || 0)
    return {
      instanceId,
      pid,
