...
function sanitizePathSegment(value: string, fallback: string) {
  const text = safeString(value).trim().replace(/[^A-Za-z0-9._:-]+/g, '_')
...
function refsPath(agentDir: string, messageId: string) {
  ...
  return path.join(indexesDir(agentDir), 'by-message-id', key.slice(0, 2), `${key}.json`)
}

function recordPath(agentDir: string, recordKey: string) {
  return path.join(recordsDir(agentDir), recordKey.slice(0, 2), `${recordKey}.json`)
}

export function buildKoishiMessageRecordKey(chatKey: string, messageId: string) {
  return hashKey(`${chatKey}\n${messageId}`)
}

export function buildStoredKoishiMessage(input: Omit<StoredKoishiMessage, 'version' | 'recordKey'>) {
...
export function saveKoishiMessage(agentDir: string, input: Omit<StoredKoishiMessage, 'version' | 'recordKey'>) {
  ...
  if (!refs.includes(relative)) {
    ...
    fs.writeFileSync(refFilePath, `${JSON.stringify([...refs, relative], null, 2)}\n`, 'utf8')
  }

  return { record, filePath }
}

export function getKoishiMessagesByMessageId(agentDir: string, messageId: string) {
  ...
  return refs
    .map((relativePath) => readJsonFile<StoredKoishiMessage | null>(path.join(storeRoot, relativePath), null))
    .filter((item): item is StoredKoishiMessage => Boolean(item && safeString(item.messageId).trim()))
}

export function getKoishiMessage(agentDir: string, chatKey: string, messageId: string) {
  const recordKey = buildKoishiMessageRecordKey(chatKey, messageId)
  return readJsonFile<StoredKoishiMessage | null>(recordPath(agentDir, recordKey), null)
}

export function updateKoishiMessage(agentDir: string, chatKey: string, messageId: string, patch: Partial<StoredKoishiMessage>) {
...
export function findKoishiMessageByChatAndId(agentDir: string, chatKey: string, messageId: string) {
  const direct = getKoishiMessage(agentDir, chatKey, messageId)
  if (direct) return direct
  return getKoishiMessagesByMessageId(agentDir, messageId).find((item) => item.chatKey === chatKey) || null
}

export function normalizeKoishiMessageLookup(agentDir: string, messageId: string, chatKey?: string) {
  const nextChatKey = safeString(chatKey).trim()
  const matches = nextChatKey
    ? (() => {
      const found = findKoishiMessageByChatAndId(agentDir, nextChatKey, messageId)
      return found ? [found] : []
    })()
...
export function normalizeElementSummary(elements: any): Array<{ type: string; attrs?: Record<string, string> }> {
  if (!Array.isArray(elements)) return []
  return elements.map((element) => {
    const attrs = element?.attrs && typeof element.attrs === 'object'
      ? Object.fromEntries(Object.entries(element.attrs).map(([key, value]) => [key, safeString(value)]).filter(([, value]) => value))
      : undefined
    return {
      type: sanitizePathSegment(safeString(element?.type).toLowerCase(), 'unknown'),
