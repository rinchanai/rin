...
import { buildPromptText, restorePromptParts, sendGenericFile, sendImageFile, sendText, sendTyping } from './transport.js'
...
export class KoishiChatController {
  ...
  constructor(app: any, dataDir: string, chatKey: string, deps: { logger: any; h: any }) {
    this.app = app
  ...
  async connect() {
    ...
    session.subscribe((event: any) => {
      switch (event?.type) {
        ...
        case 'compaction_end':
          if (this.pendingCompletedAssistantText) void this.flushInterim().catch(() => {})
          break
        case 'agent_end':
          this.pendingCompletedAssistantText = ''
          this.stopTyping()
    ...
    if (wantedSessionFile) await session.switchSession(wantedSessionFile).catch(() => {})
    if (!session.sessionManager.getSessionName?.()) await session.setSessionName(this.chatKey)
  }
  ...
  private saveState() { writeJsonFile(this.statePath, this.state) }
  private startTyping() {
    this.stopTyping()
    void sendTyping(this.app, this.chatKey, this.h)
    this.typingTimer = setInterval(() => { void sendTyping(this.app, this.chatKey, this.h) }, TYPING_INTERVAL_MS)
  }
  private stopTyping() {
  ...
  private async flushInterim(force = false) {
    ...
    await sendText(this.app, this.chatKey, `${INTERIM_PREFIX}${text}`, this.h, replyToMessageId).catch(() => {})
  }
  private nextRequestTag() {
    this.turnSeq += 1
    return `${this.chatKey}:${Date.now()}:${this.turnSeq}`
  }
  currentSessionId() { return safeString(this.session?.sessionManager?.getSessionId?.() || '').trim() }
  ...
  async resumeSessionFile(sessionFile: string) {
    ...
    if (before !== wanted) await this.session.switchSession(wanted).catch(() => {})
  ...
  async runCommand(commandLine: string, replyToMessageId = '', incomingMessageId = '') {
    ...
    if (text) await sendText(this.app, this.chatKey, text, this.h, replyToMessageId)
    return data
  }
  async runTurn(input: { text: string; attachments: SavedAttachment[]; replyToMessageId?: string; incomingMessageId?: string }, mode: 'prompt' | 'interrupt_prompt' = 'prompt') {
    ...
    if (finalText) await sendText(this.app, this.chatKey, finalText, this.h, replyToMessageId)
    for (const image of this.pendingOutboundImages.splice(0)) await sendImageFile(this.app, this.chatKey, image.path, this.h, image.mimeType || 'image/png', replyToMessageId).catch(() => {})
    for (const file of this.pendingOutboundFiles.splice(0)) await sendGenericFile(this.app, this.chatKey, file.path, this.h, file.name, replyToMessageId).catch(() => {})
  }
