...
import { chatStateDir, listChatStateFiles } from '../chat-bridge/session-binding.js'
import { buildAllowedCommandRows, drainKoishiOutbox, syncTelegramCommands } from './boot.js'
import { ensureDir, extractInboundAttachments, getChatId, lookupReplySession, persistInboundMessage, pickChatName, pickMessageId, pickReplyToMessageId, pickSenderNickname, pickUserId, safeString, wrapKoishiBridgePrompt } from './chat-helpers.js'
...
import { composeChatKey, loadIdentity, materializeKoishiConfig, trustOf } from './support.js'
...
export async function startKoishi(options: { additionalExtensionPaths?: string[] } = {}) {
  ...
  const getController = (chatKey: string) => {
    ...
    if (!controller) {
      controller = new KoishiChatController(app, dataDir, chatKey, { logger, h })
  ...
  for (const item of commandRows) {
    ...
    app.command(`${item.name} [args:text]`, item.description || '', { slash: true }).action(async ({ session }: any, argsText: any) => {
      ...
      if (replySession?.sessionFile) await controller.resumeSessionFile(replySession.sessionFile).catch(() => {})
  ...
  app.middleware(async (session: any, next: () => Promise<any>) => {
    ...
    void controller.runTurn({ text, attachments, replyToMessageId: messageId, incomingMessageId: messageId }, 'interrupt_prompt').catch((error) => {
      ...
      void sendText(app, decision.chatKey, `Koishi error: ${safeString((error as any)?.message || error || 'koishi_turn_failed')}`, h, messageId).catch(() => {})
    })
    return ''
  }, true)
  ...
  cronOutboxTimer = setInterval(() => { void drainKoishiOutbox(app, runtime.agentDir, h, logger).catch(() => {}) }, 1000)
