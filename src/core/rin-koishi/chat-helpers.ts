...
export function wrapKoishiBridgePrompt(text: string, meta: KoishiBridgePromptMeta) {
...
export function extractTextFromContent(content: any, { includeThinking = false }: { includeThinking?: boolean } = {}) {
...
export function persistInboundMessage(agentDir: string, session: any, identity: any, trustOf: (identity: any, platform: string, userId: string) => string) {
...
export function lookupReplySession(agentDir: string, chatKey: string, replyToMessageId: string) {
  ...
  return { linked, sessionId: sessionId || undefined, sessionFile: sessionFile || undefined }
}

export function markProcessedKoishiMessage(agentDir: string, chatKey: string, messageId: string, update: Record<string, unknown>) {
  updateKoishiMessage(agentDir, chatKey, messageId, update)
}

export async function persistImageParts(chatDir: string, images: Array<{ data: string; mimeType: string }>, prefix: string) {
