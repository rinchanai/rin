...
import type { ChatMessagePart, ChatOutboxPayload } from '../rin-lib/chat-outbox.js'
import { findBot, parseChatKey } from './support.js'
import type { KoishiChatState, SavedAttachment } from './chat-helpers.js'
import { ensureDir, extractTextFromContent, safeString } from './chat-helpers.js'
...
export async function sendText(app: any, chatKey: string, text: string, h: any, replyToMessageId = '') {
  const parsed = parseChatKey(chatKey)
...
export async function sendImageFile(app: any, chatKey: string, filePath: string, h: any, mimeType = 'image/png', replyToMessageId = '') {
  const parsed = parseChatKey(chatKey)
  if (!parsed) throw new Error(`invalid_chatKey:${chatKey}`)
  const bot = findBot(app, parsed.platform, parsed.botId)
  if (!bot) throw new Error(`no_bot_for_platform:${parsed.platform}${parsed.botId ? `/${parsed.botId}` : ''}`)
...
export async function sendGenericFile(app: any, chatKey: string, filePath: string, h: any, name?: string, replyToMessageId = '') {
  const parsed = parseChatKey(chatKey)
  if (!parsed) throw new Error(`invalid_chatKey:${chatKey}`)
  const bot = findBot(app, parsed.platform, parsed.botId)
  if (!bot) throw new Error(`no_bot_for_platform:${parsed.platform}${parsed.botId ? `/${parsed.botId}` : ''}`)
...
export async function sendOutboxPayload(app: any, payload: ChatOutboxPayload, h: any) {
  if (payload?.type === 'text_delivery') {
    await sendText(app, safeString(payload.chatKey).trim(), safeString(payload.text).trim(), h, safeString(payload.replyToMessageId).trim())
    return
  }
  ...
  const bot = findBot(app, parsed.platform, parsed.botId)
  if (!bot) throw new Error(`no_bot_for_platform:${parsed.platform}${parsed.botId ? `/${parsed.botId}` : ''}`)
  const nodes = (Array.isArray(payload.parts) ? payload.parts : []).map((part) => messagePartToNode(part, h)).filter(Boolean)
...
export async function attachmentToImageContent(filePath: string, mimeType = 'image/png'): Promise<ImageContent> {
  const data = await fs.promises.readFile(filePath)
  return { type: 'image', data: data.toString('base64'), mimeType }
}

export async function restorePromptParts(processing: NonNullable<KoishiChatState['processing']>) {
