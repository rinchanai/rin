...
export async function sendKoishiText(agentDir: string, payload: { chatKey: string; taskId: string; runId: string; text: string }) {
  enqueueChatOutboxPayload(agentDir, { type: 'text_delivery', createdAt: nowIso(), ...payload })
}

export async function resolveCronSessionFile(task: CronTaskRecord) {
  if (task.session.mode === 'specific' || task.session.mode === 'current') return task.session.sessionFile
  if (task.dedicatedSessionFile) return task.dedicatedSessionFile
  return undefined
}

export async function executeCronShellTask(task: CronTaskRecord, defaultCwd: string) {
...
export async function executeCronAgentTask(task: CronTaskRecord, options: { cwd: string; agentDir: string; additionalExtensionPaths?: string[] }) {
  ...
  if (task.session.mode === 'dedicated' && result.sessionFile) task.dedicatedSessionFile = result.sessionFile
  const finalText = summarizeText(result.finalText, 4000)
  return finalText || `Scheduled agent turn finished in session ${result.sessionFile || '(ephemeral)'}`
}

export async function executeCronTask(task: CronTaskRecord, options: { cwd: string; agentDir: string; additionalExtensionPaths?: string[] }) {
  const runId = cronTaskRunId(task)
  try {
    const result = task.target.kind === 'shell_command'
    ...
    task.lastError = String(error?.message || error || 'cron_task_failed').trim()
    if (task.chatKey) {
      await sendKoishiText(options.agentDir, {
        chatKey: task.chatKey,
        taskId: task.id,
