...
const rpc = await import(pathToFileURL(path.join(rootDir, 'dist', 'core', 'rin-lib', 'rpc.js')).href)
const shared = await import(pathToFileURL(path.join(rootDir, 'dist', 'core', 'rin', 'shared.js')).href)
...
test('shared resolveParsedArgs keeps passthrough and install defaults coherent', () => {
  const parsed = shared.resolveParsedArgs('', { std: true, tmux: '', tmuxList: false, user: 'demo' }, ['--std', '--foo', 'bar'])
