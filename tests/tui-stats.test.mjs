...
const stats = await import(pathToFileURL(path.join(rootDir, 'dist', 'core', 'rin-tui', 'stats.js')).href)

test('tui stats compute session stats from entries', () => {
  const entries = [
    { type: 'message', message: { role: 'user', content: [] } },
    { type: 'message', message: { role: 'assistant', content: [{ type: 'toolCall' }], usage: { input: 10, output: 20, cacheRead: 1, cacheWrite: 2, cost: { total: 0.5 } } } },
    { type: 'message', message: { role: 'toolResult', content: [] } },
  ]
  const result = stats.computeSessionStats({ contextWindow: 1000 }, '/tmp/demo', 'sid', entries, { tokens: 33, contextWindow: 1000, percent: 3.3 })
