...
const service = await import(pathToFileURL(path.join(rootDir, 'dist', 'core', 'rin-install', 'service.js')).href)

test('installer service helpers build systemd service spec', () => {
  const spec = service.buildSystemdUserService('demo', '/tmp/rin', () => '/home/demo', () => '/repo')
