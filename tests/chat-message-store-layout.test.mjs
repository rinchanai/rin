...
test("message store layout defaults to the preferred root", async () => {
  await withTempRoot(async (root) => {
    ...
    assert.deepEqual(layout.readRoots.map((item) => item.storeDir), [
      preferredStoreDir,
    ]);
...
test(
  "message store layout switches entirely to legacy when only the legacy root exists",
  async () => {
    await withTempRoot(async (root) => {
...
test(
  "message store layout keeps legacy reads as a fallback once the preferred root exists",
  async () => {
    await withTempRoot(async (root) => {
      const preferredStoreDir = path.join(root, "data", "chat-message-store");
      const legacyStoreDir = path.join(root, "data", "koishi-message-store");
