...
const stateUtils = await import(pathToFileURL(path.join(rootDir, 'dist', 'core', 'rin-tui', 'state-utils.js')).href)
const reconnect = await import(pathToFileURL(path.join(rootDir, 'dist', 'core', 'rin-tui', 'reconnect.js')).href)
...
test('rpc state utils derive branch and apply state', () => {
  ...
  stateUtils.applyRpcSessionState(target, { sessionId: 's1', sessionFile: '/tmp/x', thinkingLevel: 'low', isStreaming: true })
  ...
  assert.deepEqual(branch.map((x) => x.id), ['1', '2'])
})

test('rpc reconnect helper queues and emits status', () => {
  const events = []
  const target = { queuedOfflineOps: [], emitEvent: (e) => events.push(e), ensureReconnectLoop: () => events.push({ reconnect: true }) }
