...
const helpers = await import(pathToFileURL(path.join(rootDir, 'dist', 'core', 'rin-koishi', 'chat-helpers.js')).href)
...
test('koishi chat helpers persist outbound image parts', async () => {
  await withTempDir(async (dir) => {
    const images = [{ data: Buffer.from('demo').toString('base64'), mimeType: 'image/png' }]
