...
const { KoishiChatController } = await import(pathToFileURL(path.join(rootDir, 'dist', 'core', 'rin-koishi', 'controller.js')).href)

async function createController(chatKey = 'telegram/1:2') {
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'rin-koishi-controller-'))
...
test('koishi controller uses RpcInteractiveSession session bootstrap before first command on a fresh chat', async () => {
  ...
  controller.session = {
    ...
    setSessionName: async (name) => { namedSessions.push(name) },
  }
