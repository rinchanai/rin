...
const interactive = await import(pathToFileURL(path.join(rootDir, 'dist', 'core', 'rin-install', 'interactive.js')).href)

test('installer interactive helpers describe dir state and plan text', () => {
  const existing = interactive.describeInstallDirState('/tmp/demo', { exists: true, entryCount: 2, sample: ['a', 'b'] })
  assert.equal(existing.title, 'Existing directory')
  assert.ok(existing.text.includes('keep unknown files untouched'))

  const created = interactive.describeInstallDirState('/tmp/demo', { exists: false, entryCount: 0, sample: [] })
...
test('installer interactive helpers compute final requirements', () => {
  const elevated = interactive.buildFinalRequirements({ installServiceNow: true, needsElevatedWrite: false, needsElevatedService: true })
  assert.ok(elevated.some((line) => line.includes('use sudo/doas')))

  const local = interactive.buildFinalRequirements({ installServiceNow: false, needsElevatedWrite: false, needsElevatedService: false })
  assert.ok(local.some((line) => line.includes('skip daemon service installation')))
})
