...
const schema = await import(pathToFileURL(path.join(rootDir, 'dist', 'extensions', 'memory', 'core', 'schema.js')).href)

test('memory schema parses and renders markdown docs consistently', () => {
  const doc = schema.parseMarkdownDoc('/tmp/demo.md', `---\ntitle: Demo\nexposure: recall\ntags:\n  - one\n  - two\n---\nhello world\n`)
...
test('memory schema normalizes defaults for resident docs', () => {
  const doc = schema.normalizeFrontmatter({ exposure: 'resident', resident_slot: 'owner_identity', content: '' }, '/tmp/x.md', 'hello')
