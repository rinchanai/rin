...
/compact <instruction>     重点を指定して現在のセッションを圧縮
/reload                    拡張、プロンプト、スキル、テーマを再読み込み
/session                   現在のセッション統計を表示
```
...
updater はランチャーメタデータ、インストール manifest、管理 service file からも対象を復元できるため、クロスユーザー構成でも現在の shell が不完全に見えるだけで、実際のインストール先は見つけられることがあります。
