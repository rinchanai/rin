...
import { createTargetExecutionContext, safeString } from "./shared.js";
...
function renderTaskTrigger(task) {
...
function renderTaskTarget(task) {
...
function renderTaskSession(task) {
    const mode = safeString(task.session?.mode).trim() || "unknown";
    const sessionFile = safeString(task.session?.sessionFile).trim() ||
...
function renderCronTaskLine(task) {
    const parts = [
    ...
    if (chatKey)
        parts.push(`chat=${quoteValue(chatKey)}`);
    const lastStartedAt = safeString(task.lastStartedAt).trim();
    if (lastStartedAt)
        parts.push(`lastStartedAt=${lastStartedAt}`);
    const lastFinishedAt = safeString(task.lastFinishedAt).trim();
    if (lastFinishedAt)
        parts.push(`lastFinishedAt=${lastFinishedAt}`);
    const lastError = summarizeText(safeString(task.lastError), 120);
    if (lastError)
        parts.push(`lastError=${quoteValue(lastError)}`);
    return parts.join(" ");
}
export function renderDoctorDaemonStatusLines(daemonStatus) {
    ...
    for (const instance of Array.isArray(webSearchStatus?.instances)
        ...
        : []) {
        lines.push(`webSearchInstance=${instance.instanceId} pid=${String(instance.pid || 0)} alive=${yesNo(instance.alive)} port=${String(instance.port || "")} baseUrl=${instance.baseUrl || ""}`);
    }
    lines.push(`chatBridgeReady=${yesNo(chatStatus?.ready)}`, `chatBridgeAdapterCount=${String(chatStatus?.adapterCount ?? 0)}`, `chatBridgeBotCount=${String(chatStatus?.botCount ?? 0)}`, `chatBridgeControllerCount=${String(chatStatus?.controllerCount ?? 0)}`, `chatBridgeDetachedControllerCount=${String(chatStatus?.detachedControllerCount ?? 0)}`);
    if (!daemonStatus)
        return lines;
    ...
    lines.push(`daemonCronTaskCount=${String(taskCount)}`, `daemonCronRunningCount=${String(runningTaskCount)}`);
    lines.push(...tasks.map((task) => renderCronTaskLine(task)));
    return lines;
}
