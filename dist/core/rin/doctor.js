...
export async function runDoctor(parsed) {
    ...
    lines.push(`webSearchRuntimeReady=${webSearchStatus?.runtime?.ready ? "yes" : "no"}`, `webSearchMode=${String(webSearchStatus?.runtime?.mode || "unknown")}`, `webSearchProviderCount=${String(webSearchStatus?.runtime?.providerCount ?? 0)}`, `webSearchInstanceCount=${String(Array.isArray(webSearchStatus?.instances) ? webSearchStatus.instances.length : 0)}`);
