...
The installer also writes a user-scoped Windows Startup launcher for the daemon when the finalized plan runs on Windows, matching the existing launchd/systemd user-service boundary as closely as Windows allows without requiring a machine-wide service install. OAuth/browser-login orchestration and a richer packaged desktop shell can still be layered on this shared frontend boundary.
